def main():
    ...
    wire1_directions = wireA.split(',')
    points_a = get_points(wire1_directions)
    ...
    wire2_directions = wireB.split(',')

    points_b = get_points(wire2_directions)
    print(len(points_a))
    print(len(points_b))
    lst3 = [value for value in points_a if value in points_b] 
    print(lst3)
    part1 = min([abs(x)+abs(y) for (x,y) in lst3])
    part2 = min([points_a[p]+points_b[p] for p in lst3])
    print(part1,part2)
    return

    
def get_points(wire1_directions):
    x = 0
    y = 0
    length = 0
    locs = {}
    for direction in wire1_directions:
    # direction = wire[0]
        # print(direction)
        # for step in wireA:
        ...
        for i in range(int(delta)):
            ...
            length += 1
            coordinates = (x,y)
            if coordinates not in locs:
                locs[coordinates] = length
    return locs
    print(locs)
